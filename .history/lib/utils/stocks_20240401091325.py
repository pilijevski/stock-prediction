...
def get_meta_data(stock_symbols, stocks):
    ...
    for ticker in tqdm(stock_symbols):
        ...
        try:
            d = get_earnings_release_dates(ticker)
        except Exception as e:
            print("Error in getting earnings dates", ticker)
            continue
        ...
        for i in range(len(stock_periods) - 1, -1, -1):
            ...
            try:
                ...
                sector = np.nan
            earnings_date = pd.Timestamp(get_earnings_release_dates(ticker).loc[stock_periods[i]:].index.unique().values[0]) + DateOffset(days=1)
            print(earnings_date, stock_periods[i])
